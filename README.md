...
- `.js` をビルドせずそのまま kintone にアップロード → kintone 上で直接編集できる余地を残す
...
- **ginue で設定を Git 管理**: フィールド定義・フォームレイアウト・プロセス管理・アクセス権などを [ginue](https://github.com/goqoo-on-kintone/ginue) で pull / push して Git で追跡
...
6. `src/apps/sample1/` / `src/apps/sample2/` と `dts/sample*-fields.d.ts` は動作確認用の雛形。不要なら削除、使うなら自アプリ名にリネーム
...
│   ├── types.ts              # 全アプリ共通の汎用型（KintoneEvent / IndexEvent / DetailEvent / SubmitEvent / Global）
│   └── apps/                 # kintone JS カスタマイズ（ここを kintone にアップロード）
│       ├── sample1/          # 共通ヘルパ + 複数機能ファイルのパターン
│       │   ├── common.js     # アプリ内で共有する定数・ユーティリティ（window.__sample1Common）
│       │   ├── edit-show.js  # 編集画面の表示イベント
│       │   ├── index-show.js # 一覧画面の表示イベント
│       │   └── types.ts      # アプリ固有の型定義（JSDoc から参照）
│       └── sample2/          # 単一機能ファイルの最小パターン（common.js なし）
│           ├── edit-submit.js # 保存時バリデーション
│           └── types.ts
...
アプリごとのディレクトリの粒度は自由。`sample1/` のように複数機能ファイル + 共通ヘルパの構成と、`sample2/` のように 1 ファイルで完結させる構成を両方同梱してあります。

共有ユーティリティは 3 層に分けられる想定:

1. **全アプリ共通**: [`src/global.js`](src/global.js) を kintoneシステム管理の「JavaScript / CSSでカスタマイズ」で登録 → どのアプリからも `window.__global` で参照できる（同梱サンプルではログ関数 `log(tag, ...args)` のみ）
2. **アプリ内共通**: `src/apps/{app}/common.js` を同アプリのカスタマイズに登録 → 同一アプリの他ファイルから `window.__{app}Common` で参照（sample1 の `hasValue` / `today` がこれ）
3. **ファイル内ローカル**: IIFE ブロック内で閉じた関数（sample2 の `isValidEmail` がこれ）
...
{
  /**
   * @param {import('./types').Sample1Record} record
   */
...
型は 2 層に分けてあります。kintone の汎用イベント型（`IndexEvent<T>` / `DetailEvent<T>` / `SubmitEvent<T>` など）は [`src/types.ts`](src/types.ts) に一度だけ書き、各アプリの `src/apps/{app}/types.ts` ではアプリ固有のレコード型を食わせて `Sample1DetailEvent` のように合成するだけ。アプリ追加時に書くコードを最小化できます。

JSDoc に型リテラルを直接書き下すこともできますが、`types.ts` に一度だけ書いて `import` 参照するほうが `.js` 側の宣言が大幅に簡潔になり、型の変更が全 `.js` に自動で波及します。「TS で書ける部分は TS に、JSDoc は参照だけ」が基本方針。
